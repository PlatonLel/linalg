    ...
    for (size_t i = 0; i < m_rows; ++i) {
        for (size_t j = 0; j < m.columns(); ++j) {
            ...
            for (size_t k = 0; k < m_columns; ++k) {
                m_return[i * m.columns() + j] +=
                        static_cast<T>(m_ptr[i * m_columns + k]) * static_cast<T>(m[k * m.columns() + j]);
            }
        }
    }
...
std::string linalg::sanitize_line(const std::string &line) {
    ...
    for (char c : trimmed) {
        if (c == '|') {
            ...
            sanitized += c;
        } else if (inside_content || !std::isspace(static_cast<unsigned char>(c))) {
            sanitized += c;
        }
    }
...
void linalg::process_matrix_line(const std::string &line, size_t &cols, size_t &rows, linalg::Matrix<Complex> *matrix) {
    std::string sanitized_line = sanitize_line(line);

    if (!validate_line_format(sanitized_line)) {
        throw std::runtime_error("Invalid matrix format: rows must be enclosed in '|' and properly formatted.");
    }

    sanitized_line.erase(std::remove(sanitized_line.begin(), sanitized_line.end(), '|'), sanitized_line.end());
    size_t col_count = count_columns(sanitized_line);
    ...
    if (matrix) {
        std::istringstream row_stream(sanitized_line);
        std::string complex_str;
        size_t col = 0;

        while (row_stream >> complex_str) {
            Complex complex_num = parse_complex(complex_str);
            (*matrix)(rows, col) = complex_num;
            col++;
        }
    }
...
void linalg::load_matrix_dimensions(std::ifstream &file, size_t &rows, size_t &cols) {
    ...
    while (std::getline(file, line)) {
        process_matrix_line(line, cols, rows, nullptr);
    }
...
void linalg::load_matrix_data(std::ifstream &file, linalg::Matrix<Complex> &matrix) {
    std::string line;
    size_t cols = matrix.columns();  // Локальная копия для передачи в process_matrix_line
    size_t row = 0;

    while (std::getline(file, line)) {
        process_matrix_line(line, cols, row, &matrix);
    }
}
